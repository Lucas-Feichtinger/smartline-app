import {
   RequestSearchObjectIFace,
} from "./request.iface";
import {
   ResponseSearchObjectIFace,
} from "./response.iface";
...
export async function uf_searchObject<T>(
   ...
   for (const item of req.Dataset) {
      ...
      let searchableBlockArray: string[] = [];
      ...
      let inputWordArray = req.InputString.match(/\b\w+\b/g);
      if (inputWordArray) {
         inputWords = inputWordArray.length;

         // Iterate over each word in the change string
         outerLoop: for (let inputWord of inputWordArray) {
            // Iterate over each text block for the current item
            for (let textBlock of searchableBlockArray) {
               if (textBlock.length === 0) {
                  continue;
               }

               if (removeSeparationChars(textBlock).includes(removeSeparationChars(inputWord))) {
